...
// Modification December 2020
// Canvas 760 x 250 (900 x 250 for debug print)

function run_pipeline(canvas)
{
const YPOS = -10;
const BUTTON_X =  70;
const BUTTON_Y = 200;
...
need = (contents[stage[1]]>>8) & 0xF;
//if (need == 0xF) need = -1;		// For obviousness
//else
  {
  if (((stage[2] >= 0) && (need == ((contents[stage[2]]>>4) & 0xF)))
   || ((stage[3] >= 0) && (need == ((contents[stage[3]]>>4) & 0xF)))
   || ((stage[4] >= 0) && (need == ((contents[stage[4]]>>4) & 0xF))))
    {
    stage[2] = alloc();			// Create bubble
    contents[stage[2]] = SHADOW;
    stalled = true;
    }
  else
    {
...
if (stalled)
  {
  ctx.fillStyle = "Red";
  ctx.fillText("Stalled", 265, YPOS + 55);
  ctx.fillText("'Bubble' created", 405, YPOS + 55);
